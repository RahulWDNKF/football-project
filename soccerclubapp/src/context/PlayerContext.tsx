...
import {
  ...
  updateTaskApi, deleteTaskApi, createEmailApi,
  updateEmailApi,
  deleteEmailApi,
  createDocumentApi,
  deleteDocumentApi,
      ...
      addEmail: async (e) => {
        try {
          const created = await createEmailApi({
            playerId: e.playerId ? String(e.playerId) : undefined,  // ✅ ensure string
            clubId: e.clubId ? String(e.clubId) : undefined,
          ...
          showSuccess('Email Created', `Email to ${created.recipientEmail} created successfully`);
        } catch (err) {
          console.error('Create email API failed', err);
          showError('Error', 'Failed to create email');
