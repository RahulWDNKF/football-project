...
const Tasks = () => {
  ...
  const getEntityName = (task: Task) => {
    if (task.playerId === 'player') return players.find(p => p.id === task.clubId)?.fullName;
    return clubs.find(c => c.clubId === task.clubId)?.clubName;
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-3">
        {filtered.map(task => {
          ...
          return (
            <Card key={task.taskId} className={task.status === 'completed' ? 'opacity-60' : overdue ? 'border-destructive/40' : ''}>
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <button onClick={() => !isPlayer && toggleStatus(task)} className="mt-0.5" disabled={isPlayer}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground flex-wrap">
                      ...
                      {getEntityName(task) && (
                        <Link to={task.playerId === 'player' ? `/players/${task.clubId}` : `/clubs/${task.clubId}`}
...
const AddTaskDialog = ({ players, clubs, scouts, onAdd }: { players: any[]; clubs: any[]; scouts: any[]; onAdd: (t: Task) => void }) => {
  ...
  const entities = entityType === 'player' ? players.map(p => ({ id: p.id, name: p.fullName })) : clubs.map(c => ({ id: c.clubId, name: c.clubName }));
